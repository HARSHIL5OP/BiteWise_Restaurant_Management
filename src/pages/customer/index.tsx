...
export default function CustomerHome() {
  ...
  return (
    <div className="bg-[#0A0F1C] min-h-screen text-slate-200 pb-20 font-sans max-w-md mx-auto shadow-[0_0_50px_rgba(0,0,0,0.5)] border-x border-slate-900 overflow-x-hidden relative scroll-smooth selection:bg-orange-500/30">
      ...
      <div className="bg-slate-900/50 py-6 my-2 border-y border-slate-800">
        <SectionHeader title="Harshil, what's on your mind?" subtitle="Curated specifically for your taste buds." />
        <div className="px-4 grid grid-cols-2 gap-3">
          <div className="bg-gradient-to-br from-[#1e293b] to-slate-900 p-4 rounded-2xl border border-slate-800 shadow-lg active:scale-95 transition-transform flex flex-col justify-end min-h-[120px] relative overflow-hidden group">
            ...
            <h4 className="text-sm font-bold text-white z-10 w-3/4">Restaurants near me</h4>
            <div className="w-6 h-6 bg-slate-800 rounded-full mt-2 flex items-center justify-center z-10">
          ...
          <div className="bg-gradient-to-br from-[#1e293b] to-slate-900 p-4 rounded-2xl border border-slate-800 shadow-lg active:scale-95 transition-transform flex flex-col justify-end min-h-[120px] relative overflow-hidden group">
            ...
            <h4 className="text-sm font-bold text-white z-10 w-3/4">Pre-book exclusive offers</h4>
            <div className="w-6 h-6 bg-slate-800 rounded-full mt-2 flex items-center justify-center z-10">
      ...
      <div className="px-4 flex flex-wrap gap-2 mb-8">
        {LOCATIONS.map((loc, idx) => (
          <div key={idx} className="px-4 py-2 rounded-full border border-slate-700 bg-slate-800/40 text-sm font-medium text-slate-300 hover:bg-slate-700 hover:text-white transition-colors cursor-pointer active:scale-95 shadow-sm">
            {loc}
          </div>
        ))}
      </div>
